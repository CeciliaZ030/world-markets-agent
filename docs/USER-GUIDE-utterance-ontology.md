...
- Speech-only opener repair: a transcript that starts with a number (`5.05 ETH`, `5 ETH`) and has no command/question word → `buy 5 WETH`. “buy 5” is often fused to `5.05` because *buy* and *five* share a diphthong. Typed `5.05 ETH` is unchanged. Leading `I have` / `I've` / `by` / `wait` on a sized trade is treated as `buy`. Leading `well` / `cell` / `sale` on a sized trade is treated as `sell`. Typed `I have 50 ETH` and `well 50 ETH` are unchanged.
